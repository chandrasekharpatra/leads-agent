...
export default defineEventHandler(async (event: H3Event) => {
  ...
  const response = await env.LEADS_SERVER.fetch(
    `https://leads.example.com/v1/leads?pincode=${query.pincode}`,
    {
      method: "GET",
      headers: {
  ...
  const csvHeaders = [
    ...
    "Address",
    "Pincode",
    "Has Toastmaster Club",
    "Toastmaster Club URL",
    "Employee Count",
    "HR Profiles",
  ];
  const csvRows = responseBody.leads.map((lead) => {
    const hrProfiles = lead.data.hrProfiles
      .map((profile) => `${profile.name} (${profile.linkedInUrl || "N/A"})`)
      .join("; ");
    return [
      lead.companyId,
      lead.data.name,
